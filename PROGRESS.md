...
## 2026-09-05 인스타그램 업로드 실패 원인 조사 및 수정

`EconCardNews-Publish`(18:00) 실행 후 3개 카테고리 전부에서 텔레그램에
"⚠️ 인스타그램 업로드 실패 ... Client error '400 Bad Request' for url
'.../media'"만 오고 실제 계정에는 게시물이 하나도 안 올라간 문제 발생.

- **카드 생성 자체는 정상**(3개 카테고리 모두 카드세트는 만들어짐) — 실패
  지점은 `publish_instagram.py`의 인스타그램 업로드 단계뿐.
- 3개 카테고리가 전부 동일한 시점에 동일한 400 에러로 실패한 패턴은 카테고리별
  콘텐츠 문제가 아니라 **공용 자원(액세스 토큰) 문제**임을 시사. `INSTAGRAM_ACCESS_TOKEN`은
  9/3 라이브 테스트 때 발급한 토큰인데, PROGRESS.md 자체에 "장기 토큰 교환은
  권장 사항"이라고만 적혀있고 실제로 교환했다는 기록은 없어, 단기(수 시간
  ~하루 수준) 토큰이 그대로 만료됐을 가능성이 유력한 원인.
- 다만 `httpx`의 `resp.raise_for_status()`가 상태 코드만 담고 Graph API가
  응답 본문에 실어 보내는 실제 사유(`error.message`/`error.code`, 예:
  토큰 만료면 code 190)는 버려버려서, 텔레그램 알림만으로는 "토큰 만료"인지
  다른 이유인지 구분할 수 없었음(운영자가 원인 파악을 못 하고 있었던 근본 이유).

### 수정한 것

1. `instagram_client.py`: 모든 Graph API 호출의 `raise_for_status()`를
   `_raise_with_detail()`로 교체 — 실패 시 Graph API가 준 실제
   `message`/`code`/`type`을 그대로 예외 메시지에 담아, 다음부터는 텔레그램
   알림만 보고도 정확한 원인(토큰 만료 vs 다른 오류)을 바로 알 수 있게 함.
2. `instagram_client.verify_token()` 추가 + `publish_instagram.main()`이
   카테고리별 업로드를 시작하기 전에 먼저 한 번 호출하도록 변경 — 토큰이
   죽어있으면 카테고리 수만큼 똑같이 알 수 없는 에러를 반복 발송하는 대신,
   "토큰이 유효하지 않다"는 알림 1건만 보내고 업로드 단계 전체를 건너뜀
   (카드는 `cards.published=false`로 남으므로 토큰만 갱신하면
   `python publish_instagram.py` 재실행으로 소급 업로드 가능, 데이터 손실 없음).

### 운영자가 해야 하는 일

`INSTAGRAM_ACCESS_TOKEN`을 재발급해야 함(9/3 진행상황에 적힌 "확장판 1번"의
4~6번 단계 반복: Graph API Explorer에서 새 토큰 발급 -> 가능하면 60일
장기 토큰으로 교환 -> `.env`의 `INSTAGRAM_ACCESS_TOKEN` 갱신). 이번 수정
이후에는 토큰이 다시 만료돼도 텔레그램 알림에 정확한 사유가 찍히므로
같은 원인으로 헤매는 일은 없어짐.

## 2026-09-03 대규모 재설계 — "묘한 경제" 브랜드로 전면 교체

운영자가 실제 계정(@myohan.economy = "묘한 경제")을 보유하고 있었고, 이미
고양이 마스코트+파스텔 브랜드가 완성된 예전 프로젝트(`H:\내 드라이브\obsidian\
카드뉴스 자동화_경제`)가 있다는 걸 확인. 9/1~9/2에 만들었던 FLUX 배경+카드
1장짜리 디자인은 이 브랜드와 전혀 안 맞아 폐기하고, `/grill-me`로 방향을
확정한 뒤 전면 재설계함(결정 사항은 대화 로그 참고, 핵심만 요약):

- **카드 1장 → 3~5장 캐러셀**: 표지 + 핵심 1~3장 + 고지. 하루 3게시물(카테고리당
  1개) 구조는 유지.
- **FLUX/비전검증 완전 제거**: 예전 브랜드는 파스텔 배경+고양이 일러스트만
  쓰고 AI 배경이미지가 없음. `generate_image.py`, `verify_image.py`,
  `compose_card.py`, `generate_prompt.py` 전부 삭제.
- **렌더링을 Pillow → Node+Playwright로 교체**: 예전 프로젝트의
  `card-template-v2.html`/`render.js`/`validate.js`를 그대로 재사용
  (`assets/templates/`, `assets/character/`로 이관). 파이썬이 content.json을
  만들면 Node 프로세스를 호출해 렌더.
- **RSS 제목만 → 기사 원문 스크래핑**: `fetch_article.py` 신규(trafilatura).
  Google News RSS 링크는 JS 리다이렉트라 실제 언론사 URL을 얻으려면
  헤드리스 브라우저가 필요해 `assets/templates/resolve_url.js` 추가.
- **카드 콘텐츠 생성**: `generate_cardset.py` 신규 — 원문 근거 LLM 생성 +
  검증 + 렌더 + (실패 시) 최소 폴백까지 담당. 자세한 안전장치는 아래
  "카드세트 생성 관련 설계 결정" 참고.
- **인스타 업로드를 단일 이미지 → 캐러셀로 교체**: `instagram_client.py`에
  `upload_and_publish_carousel()` 추가, `publish_instagram.py`가 폴더 안
  번호 PNG 전체를 캐러셀 하나로 올리도록 재작성.
- **Node 의존성 위치 주의**: `node_modules`를 프로젝트 폴더(Google Drive
  동기화 대상)에 두면 파일이 깨지는 문제를 실측으로 확인(playwright
  package.json이 빈 파일로 저장됨). `~/.cardnews-deps/`에 설치하도록 변경
  — `render.js`에 이미 있던 폴백 경로 로직이 정확히 이 문제를 겨냥한 것으로
  보임(예전 개발자도 같은 문제를 겪었던 듯).

### 카드세트 생성 관련 설계 결정 (`generate_cardset.py`)

소형 무료 모델(`llama-3.1-8b-instruct`)은 글자수 제한·JSON 형식·시각화
스키마를 안정적으로 못 지킨다는 걸 실측으로 여러 번 확인해서, 3단계
안전장치를 뒀다(제목 15자 요약 때와 같은 "LLM=품질, 코드=제약" 원칙):

1. `sanitize_cards()`: hook/title/body 강제 절삭, 존재하지 않는 포즈면
   `cat-default`로 대체, `visual` 필드 형식이 안 맞으면 통째로 제거, 고지
   카드 문구는 매번 LLM에게 맡기지 않고 고정값으로 강제(validate.js가
   문구 정확 일치를 요구해서 조금만 달라도 실패했음), `stat` 필드는 128px
   폰트 표지 요소라 한글이 2자 이상 섞이거나 6자를 넘으면 통째로 제거
   (실측: "16개 부문" 같은 한글 stat이 카드 밖으로 넘치는 오버플로를
   반복적으로 일으킴).
2. 검증(`validate.js`) 실패든 렌더(`render.js`의 자체 overflow 검사) 실패든
   최대 3회(`MAX_ATTEMPTS`) 콘텐츠를 다시 생성해 재시도.
3. 그래도 실패하면 `build_fallback_content()`로 제목+원문 첫 문장만 써서
   구조 규칙을 100% 만족하는 카드 3장(표지+설명+고지)을 코드로 직접 생성
   — 카테고리가 그날 통째로 비지 않도록 하는 최후 안전망.

`_call_text_model`이 JSON 출력을 요청하면 모델이 문자열이 아니라 이미
파싱된 dict/list를 돌려줄 때가 있다는 것도 실측으로 확인, `llm_client.py`가
이 경우 `json.dumps`로 문자열로 통일해 반환하도록 처리.

### 실사용 검증 (2026-09-03)

승인된 뉴스 2건(stock: 현대차그룹 디자인상 수상, index_macro: 코스피
장중 출렁)으로 각각 실제 카드 5장씩 렌더 완료. 배지 문구·포즈 선택
(부정적 뉴스에 cat-worried, 설명 카드에 cat-explain 등)·표지 부제 요약이
브랜드 톤에 맞게 나오는 것 확인. **인스타그램 실제 캐러셀 게시도 라이브
테스트 완료** — stock(media_id 17962994706176334)과 index_macro
(media_id 18113319331805110) 각 5장씩 실제 계정(@myohan.economy)에
정상 게시됨.

### 이월 큐(get_allowed_quota) 관련 버그 수정

`compose_card.py`(당시)의 `get_allowed_quota()`가 발행 이력이 아예 없는
첫날에도 "3일 밀렸다"로 오인해 쿼터를 4로 잘못 계산하는 버그를 발견.
카테고리에 카드 발행 이력이 전혀 없으면 이월 계산을 하지 않도록
(`have_dates`가 비어있으면 quota=1) 수정. `webhook/api/index.py`의
동일 로직도 같은 패턴이라 확인 필요(REST 버전은 처음부터 이 수정 반영해서
작성함).

### 오늘 있었던 사고와 안전장치 추가

`compose_card.py`를 디자인 반복 테스트하며 하루에 여러 번 실행했는데,
그때마다 카테고리당 카드가 계속 쌓였고(`cards` 테이블에 언제든 새 행이
insert됨), 이후 `publish_instagram.py`를 실행하자 그 미발행 카드 9장이
전부 한 번에 실제 인스타그램에 올라감(같은 뉴스의 디자인 반복 버전들이
중복 게시됨). 원인은 "재실행해도 오늘 이미 만든 카드 수만큼은 더 안 만든다"는
안전장치가 없었기 때문 — `count_cards_today()` 체크를 추가해 quota를
넘는 재실행을 막도록 수정함(같은 원칙이 `generate_cardset.py`에도 자연히
적용됨: `news_item_id`별로 이미 `cards` 행이 있으면 재생성하지 않음).
Instagram Graph API는 이미 게시된 피드 게시물을 프로그램으로 삭제하는
기능이 없어, 잘못 올라간 8건은 운영자가 앱에서 직접 삭제 처리함.

### 같은 날 마무리한 것 (라이브 테스트 + 스케줄 재개 + 포즈 생성)

카드 시각화 결과를 보여드린 뒤 운영자가 "3가지 모두 지금 해결"을 요청해
같은 세션에서 이어서 처리함:

1. **인스타그램 실제 캐러셀 게시 라이브 테스트 완료** — 다만 그 전에
   중요한 버그를 하나 더 발견함: 앞서 `compose_card.py`를 반복 테스트하며
   생긴 낡은 단일-이미지 카드 3건(id 10~12, 옛 news_item 1154/2492/4081
   가리킴)이 `cards` 테이블에 미발행 상태로 남아있었고, 정작 새로 만든
   진짜 카드세트(6465/7155)는 테스트 중 `process_item()`을 직접 호출해서
   DB insert를 거치지 않아 테이블에 기록조차 안 돼 있었음. 낡은 3건 삭제
   후 `generate_cardset.main()`을 제대로 실행해 재생성 -> 정식으로 기록된
   2건(stock, index_macro)을 `publish_instagram.py`로 게시. 각 5장씩
   실제 @myohan.economy 계정에 정상 게시 확인(media_id는 위 요약 참고).
2. **`EconCardNews-Publish` 작업 스케줄러 재활성화 완료** — 다음 실행
   2026-09-04 18:00.
3. **없는 고양이 포즈 AI 자동 생성 구현 완료** — `generate_pose.py` 추가,
   실제 생성 테스트로 검증(자세한 내용은 아래 새 절 참고).

### 새 포즈 자동 생성 관련 설계 결정 (`generate_pose.py`)

- 처음엔 "순수 그린(#00FF00) 배경"을 프롬프트로 요청하고 RGB 유클리드
  거리로 투명 처리했는데, 생성 모델이 단색 대신 그라데이션/비네트가 섞인
  초록을 주는 경우가 많아(실측 확인) 제대로 안 지워짐. **HSV 색상(Hue)
  기준**으로 "초록 계열이면" 넓게 잡아내도록 바꿔서 해결(밝기/채도가
  달라도 색상각만 맞으면 투명 처리). 캐릭터는 크림/갈색/분홍 계열이라
  초록 색조가 없어 안전.
- char_new 설명에 초록색 소품(예: "초록 화살표")을 넣으면 캐릭터 소품까지
  같이 지워지는 부작용을 실측으로 발견 — 프롬프트에 "고양이 포즈·표정만
  묘사, 초록색 금지" 안내를 추가해 대응.
- 같은 설명으로 이미 만든 포즈 파일이 있으면 재생성하지 않고 재사용
  (`out_path.exists()` 체크).
- 기존 18종처럼 손으로 다듬은 결과와 그림체가 100% 동일하다는 보장은
  없음(AI 이미지 생성의 근본적 한계) — 사용자가 이 리스크를 인지하고
  "바로 자동 사용"하기로 결정.

### 남은 이슈

1. **overflow 재시도가 생각보다 자주 발생**(실측 4건 중 3건이 1차 시도에서
   표지 stat-hero 오버플로 발생, 재시도로 통과) — `stat` 필드 제거 기준
   (한글 2자 이상 또는 6자 초과)을 더 보수적으로 낮추는 걸 고려할 것.
   지금은 재시도+최소 폴백으로 커버됨.
2. **날씨 API 연동은 하지 않기로 함**(달력상 계절만 자동 전환) — 사용자
   확인된 결정.
3. **`image_prompts`, `generated_images` Supabase 테이블은 이제 아무것도
   안 씀**(FLUX 파이프라인 삭제로 인한 레거시). 삭제하지 않고 그대로
   두었으니 정리하고 싶으면 Supabase 대시보드에서 DROP TABLE 가능.
4. **오늘 잘못 게시된 인스타그램 게시물 8건**은 운영자가 직접 앱에서 삭제
   처리하기로 함(API로는 불가능).
5. **`generate_cardset.py`나 다른 스크립트를 테스트할 때 `main()`을 거치지
   않고 내부 함수(`process_item()` 등)를 직접 호출하면 DB에 기록이 안 남는다**
   — 이번에 실제로 헷갈렸던 부분이라 다음에도 테스트할 때 유의할 것.

## 이전 진행상황 (2026-09-01, MVP + 확장판)
