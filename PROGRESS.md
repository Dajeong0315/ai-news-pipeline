...
브랜드 톤에 맞게 나오는 것 확인. 인스타그램 실제 캐러셀 게시는 **아직
실제 라이브 테스트 전**(오늘 있었던 중복 게시 사고 이후라 운영자 확인
후 진행 예정, 아래 "남은 이슈" 참고).
...
1. **인스타그램 실제 캐러셀 게시 라이브 테스트 필요** — `upload_and_publish_carousel()`은
   코드 리뷰/문법 확인만 했고 실제 Instagram Graph API로 캐러셀을 올려본
   적은 아직 없음. 운영자 확인 후 1건만 먼저 테스트 권장.
2. **`EconCardNews-Publish` 작업 스케줄러 작업이 현재 비활성화 상태**(오늘
   사고 직후 꺼둠). 위 라이브 테스트가 끝나면 `Enable-ScheduledTask
   -TaskName "EconCardNews-Publish"`로 다시 켜야 자동 발행이 재개됨.
3. **overflow 재시도가 생각보다 자주 발생**(실측 2건 중 2건 모두 1차
   시도에서 표지 stat-hero 오버플로 발생, 2차 시도에서 통과) — `stat` 필드
   제거 기준(한글 2자 이상 또는 6자 초과)을 더 보수적으로 낮추는 걸 고려할
   것. 지금은 재시도로 커버되지만 `MAX_ATTEMPTS`(3) 안에 항상 성공한다는
   보장은 없음(그래서 최소 폴백을 만들어 뒀음).
4. **날씨 API 연동은 하지 않기로 함**(달력상 계절만 자동 전환) — 사용자
   확인된 결정.
5. **없는 고양이 포즈를 AI로 새로 만드는 기능은 아직 미구현** — grill-me에서
   "필요하면 AI로 생성해 바로 자동 사용"하기로 결정했으나, 이번 세션에서는
   핵심 파이프라인(원문 스크래핑~렌더~캐러셀 게시)을 완성하는 데 집중해
   포함하지 못함. 지금은 LLM이 기존 18종 포즈 중에서만 고르고, 어울리는
   포즈가 없어도 `cat-default`로 대체됨. 다음 세션에서 이어서 구현 필요
   (Cloudflare 이미지 모델로 브랜드 캐릭터 설명 기반 생성 + 배경 제거(크로마키)
   방식으로 접근하는 게 현실적일 것).
6. **`image_prompts`, `generated_images` Supabase 테이블은 이제 아무것도
   안 씀**(FLUX 파이프라인 삭제로 인한 레거시). 삭제하지 않고 그대로
   두었으니 정리하고 싶으면 Supabase 대시보드에서 DROP TABLE 가능.
7. **오늘 잘못 게시된 인스타그램 게시물 8건**은 운영자가 직접 앱에서 삭제
   처리하기로 함(API로는 불가능).
